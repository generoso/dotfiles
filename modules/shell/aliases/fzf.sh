...
function qfzf_code_with() {
  code "$(find $(_typical_dirs) -type f $find_args 2>/dev/null | egrep "$egrep_filter" | sort -r -u | _fzf --header="EDIT..." --preview="head -100 {}" --bind="left:toggle-preview")"
}
...
function qfzf_file_path_egrepargs_X_by_filenamecontent() {
  ...
  local header="$3"
  f="$(find $from -type f $find_args $find_extra_args -exec stat -f %m:%N {} + | egrep "$egrep_filter" | sort -n -r | uniq | awk -F: '{print $2}' | while IFS= read -r f ; do echo "$f: $(head -$_FILE_CONTENT_MAX_FZF "$f" | tr -d '\n' | tr -d '\0')"; done | _fzf --header="$header" --tiebreak=index | awk -F: '{print $1}')"
  if [ "$f" != "" ]
  then
    code $f
  else
